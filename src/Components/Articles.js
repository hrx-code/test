...
const UploadNews = () => {
  ...
  useEffect(() => {
    ...
    const newsLimit =1;
    const dbCollection = collection(db, selectedDB);
    ...
    console.log("db selected to push news : "+ selectedDB);
    fetchData();
  }, [selectedDB])
  ...
  const PushNewsToFirebase=async(e)=>{
    ...
    const result =await setDoc(doc(db, selectedDB,`${Title}`), 
      data
    ...
    <div className='d-flex flex-column align-items-center justify-content-center'>
        ...
        <form className='form' onSubmit={PushNewsToFirebase}>
          <div>
            ...
            value={selectedDB} onChange={handleSelectedDB}>
              <option selected value=''>Please Select One</option>
              <option value='Anonymous'>Exp:Anonymous</option>
              <option value='Articles'>Art:Articles</option>
            </select>
          </div>
